import torch.optim
from Data import ValGenerator, ATGMDataset
...
def vis_and_save_heatmap(model, input_img, text, labs, vis_save_path):
    model.eval()
...
if __name__ == '__main__':
    ...
    if config.task_name == "MoNuSeg":
        ...
        test_num = 1568
        model_type = config.model_name
        model_path = "./Synapse/" + model_type + "/" + test_session + "/models/best_model-" + model_type + ".pth.tar"

    elif config.task_name == "ACDC":
        test_num = 376
        model_type = config.model_name
        model_path = "./ACDC/" + model_type + "/" + test_session + "/models/best_model-" + model_type + ".pth.tar"
    ...
    test_dataset = ATGMDataset(config.test_dataset, config.task_name, test_text, tf_test, image_size=config.img_size)
