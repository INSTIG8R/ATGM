...
class AttentionGate(nn.Module):
    def __init__(self,gate_input_channel,skip_input_channel):
        ...
        self.Wg = nn.Sequential(
            nn.ConvTranspose2d(in_channels=gate_input_channel,out_channels=skip_input_channel,kernel_size=2,stride=2),
            nn.BatchNorm2d(skip_input_channel)
        )
        self.Ws = nn.Sequential(
            nn.Conv2d(in_channels=skip_input_channel,out_channels=skip_input_channel,kernel_size=3,padding=1),
            nn.BatchNorm2d(skip_input_channel)
        )
        self.relu = nn.ReLU(inplace=True)
        self.output = nn.Sequential(
            nn.Conv2d(skip_input_channel, skip_input_channel, kernel_size=1, padding=0),
            nn.Sigmoid()
        )
